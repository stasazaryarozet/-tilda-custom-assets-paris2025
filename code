...
1.  **Замените плейсхолдеры:**
    * В JavaScript, в строке `var authorBlockSelectors = ['#rec1036890941', '#rec1036956216'];` замените `#rec...` на **актуальные ID ваших блоков с портретами авторов**.
2.  **Разместите этот файл** (например, `moj-custom-code.js`) на GitHub Pages или другом хостинге.
3.  **Получите прямую HTTPS ссылку** на этот файл.
4.  В Tilda, в **Настройки сайта > Еще > HTML-КОД ДЛЯ ВСТАВКИ В HEAD**, удалите все предыдущие отдельные `<style>` и `<script>` теги для этих кастомизаций и вставьте **только одну строку**:
    ```html
    <script src="ПОЛНАЯ_HTTPS_ССЫЛКА_НА_ВАШ_moj-custom-code.js" defer></script>
    ```
5.  **Сохраните изменения в Tilda и переопубликуйте весь сайт.**
6.  **Тщательно протестируйте** все функции на разных устройствах и при разных сценариях прокрутки.

Теперь у вас будет один файл для управления всеми этими кастомными стилями и скрипта
